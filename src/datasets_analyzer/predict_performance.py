...
class OfflineRLMetaPredictor:
    def __init__(self):
        ...
        self.model = GradientBoostingRegressor(n_estimators=100, max_depth=2, random_state=42)
        self.scaler = StandardScaler()
        self.is_trained = False
        
    def train(self, registry_csv_path):
        if not os.path.exists(registry_csv_path):
            raise FileNotFoundError(f"Registry file not found at {registry_csv_path}")
        ...
        print(f"[✓] Meta-Predictor successfully trained on {len(df_clean)} dataset environment tiers.")
    ...
    def predict_dataset(self, profile_json_path):
        ...
        if not os.path.exists(profile_json_path):
            raise FileNotFoundError(f"Profile file missing: {profile_json_path}")
    ...
    def cross_validate_with_real_world(self, cql_runs_dir, profiles_dir):
        ...
        search_pattern = os.path.join(cql_runs_dir, "**", "global_pipeline_report.json")
        report_paths = glob.glob(search_pattern, recursive=True)
        
        if not report_paths:
            print(f"[!] No pipeline execution reports found under {cql_runs_dir}")
            return
        ...
        for report_path in report_paths:
            ...
            raw_env_string = report.get("environment", "") # e.g., "mujoco/walker2d/simple-v0"
            real_world_score = report.get("aggregate_performance", {}).get("mean_d4rl_score", None)
            ...
            profile_filename = profile_filename.replace("medium_replay", "medium-replay")
            
            profile_path = os.path.join(profiles_dir, profile_filename)
            
            if not os.path.exists(profile_path):
                print(f"[!] Found execution results for {raw_env_string}, but feature profile is missing at {profile_path}. Skipping...")
                continue
        ...
        print(f"   Overall Meta-Predictor Mean Absolute Error (MAE): {mean_absolute_error:.4f}")
        print("=========================================================================\n")
