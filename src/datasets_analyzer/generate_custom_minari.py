...
def create_local_medium_replay():
    ...
    print("=========================================================")
    print(f" Generating Custom Local Minari Dataset: {dataset_id}")
    print("=========================================================")

    # Clear out any corrupted or pre-existing local copy of the target ID
    if dataset_id in minari.list_local_datasets():
        print("[!] Found existing local dataset artifact. Deleting before re-run...")
    ...
    num_episodes = 200
    print(
        f"[+] Simulating {num_episodes} custom trajectories into the data collector...")
