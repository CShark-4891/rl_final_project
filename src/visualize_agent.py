...
def visualize(model_path: str, env_name: str, render_mode: str = "human", num_episodes: int = 3, record_video: bool = False, video_folder: str = "../results/videos"):
    ...
    print(f"[+] Initializing Gymnasium ID: {gym_id} (Render Mode: {actual_render_mode})")

    if gym_id == "Ant-v4":
        # Enable contact forces slicing if trained with 105 dims
        base_env = gym.make(gym_id, render_mode=actual_render_mode, use_contact_forces=True)
    ...
    if record_video:
        ...
        print(f"[+] Video recording activated! Output directory: {video_folder}")
    ...
    for ep in range(num_episodes):
        ...
        print(f" [+] Episode {ep + 1}/{num_episodes} finished | Total Return: {episode_return:.2f} | Steps: {step_count}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Render or record evaluation episodes of a trained policy.")
    parser.add_argument("--model_path", type=str, required=True, help="Path to saved .d3 model file")
    parser.add_argument("--environment", type=str, required=True, help="Dataset/Environment identifier string")
    parser.add_argument("--mode", type=str, choices=["human", "rgb_array"], default="human", help="Render mode (human for GUI window, rgb_array for background rendering)")
    parser.add_argument("--record", action="store_true", help="Record video (.mp4) instead of GUI rendering")
    parser.add_argument("--episodes", type=int, default=3, help="Number of visualization episodes")
